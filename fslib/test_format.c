...
int main() {
    printf("=== VSFS Filesystem Formatting Tests ===\n\n");
    ...
    // All tests passed
    printf("=== Test Summary ===\n");
    printf("🎉 All formatting tests passed!\n");
    return 0;
}
...
int test_different_disk_sizes() {
    printf("size of inode_t: %lu\n", sizeof(inode_t));
    assert(sizeof(inode_t) == 320); // if inode size changes some tests will be inaccurate
    const char *disk_name = "test_disk_sizes";
    
    printf("  Beginning tests with block size = %d, inode size = %lu\n\n", BLOCK_SIZE, sizeof(inode_t));
    ...
    struct {
        ...
        {BLOCK_SIZE * 2, 10, false},    
        {BLOCK_SIZE * (3 + 5), 60, true},  // each block fits 12.8 inodes, so 12*5=60 
        {BLOCK_SIZE * (3 + 5), 64, false}, // it should not be able to fit 12.8*5=64 
    ...
    }; unsigned int num_test_cases = 9;
    
    // Validate test cases using helper function
    for (unsigned int i = 0; i < num_test_cases; i++) {
        ...
        if (test_cases[i].should_succeed && available_blocks < min_blocks) {
            printf("    ⚠️  Test case %d: disk has %zu blocks but needs %zu blocks for %zu files\n", 
                   i, available_blocks, min_blocks, test_cases[i].max_files);
        } else if (!test_cases[i].should_succeed && available_blocks >= min_blocks) {
            printf("    ⚠️  Test case %d: disk has %zu blocks and needs %zu blocks for %zu files (should fail but might succeed)\n", 
                   i, available_blocks, min_blocks, test_cases[i].max_files);
        }
    }
    ...
    for (unsigned int i = 0; i < num_test_cases; i++) {
        ...
        size_t num_inode_table_blocks = ceildiv(test_cases[i].max_files, BLOCK_SIZE / sizeof(inode_t));
        size_t num_data_bitmap_blocks = ceildiv(min_blocks, BLOCK_SIZE * 8);
        
        printf("  Test %d: disk size = %zu, thus number of blocks = %zu, max files = %zu\n", 
        ...
        printf("    Requires: %zu superblock, %zu inode bitmap blocks (ceil(%zu / %zu)), %zu data bitmap blocks (ceil(%zu / %zu)), %zu inode table blocks (ceil(%zu / %zu))\n",
               ...
               num_inode_table_blocks, test_cases[i].max_files, (size_t)BLOCK_SIZE / sizeof(inode_t));
...
int test_root_directory_creation(const char *disk_name) {
    ...
    if (strcmp(root_inode.name, "root") != 0) {
        printf("    ✗ Root inode name should be root, got %s\n", root_inode.name);
        close(fd);
        return -1;
    }
    ...
    if (root_inode.nlinks != 0) {
        printf("    ✗ Root inode nlinks should be 0, got %u\n", root_inode.nlinks);
        close(fd);
        return -1;
    }

    if (root_inode.is_directory != true) {
        printf("    ✗ Root inode is_directory should be true, got %d\n", root_inode.is_directory);
        close(fd);
        return -1;
    }

    if (root_inode.is_allocated != true) {
        printf("    ✗ Root inode is_allocated should be true, got %d\n", root_inode.is_allocated);
        close(fd);
        return -1;
    }
...
size_t calculate_minimum_blocks(size_t max_files) {
    ...
    size_t num_inode_table_blocks = ceildiv(max_files, BLOCK_SIZE / sizeof(inode_t));
